...
def _(np, pd):
    ...
    # ------------------------------------------------------------
    path = "/Users/sazisbekuu/Downloads/ShtukaResearch/Data2023-2024[perRound].csv"    # adjust path if needed
...
def _(
    ...
    def simulation_plots():
        ...
        # Plot on second axis
        p2 = np.percentile(I_paths, 2.5, axis=0)
        p25 = np.percentile(I_paths, 25, axis=0)
        p50 = np.percentile(I_paths, 50, axis=0)
        p75 = np.percentile(I_paths, 75, axis=0)
        p97 = np.percentile(I_paths, 97.5, axis=0)

        ax2.fill_between(range(0, horizon + 1), p2, p97, color='skyblue', alpha=0.4, label='95% interval')
...
def _(
    ...
    simulation_parameters_control = mo.vstack([
        ...
        mo.hstack([dropdown_block_size, radio_residual, switch_differencing])
    ], gap = 'xs', align='start')
...
def _(
    ...
    risk_admissibility_parameters_control = mo.vstack([
        mo.hstack([slider_Plow, slider_Plow.value]),
        mo.hstack([slider_Phigh, slider_Phigh.value]),
        mo.hstack([slider_Tstar, slider_Tstar.value]),
        mo.hstack([slider_Ttail, slider_Ttail.value]),
        mo.hstack([slider_Teps, slider_Teps.value]),
